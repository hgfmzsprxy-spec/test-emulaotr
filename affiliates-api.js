...
module.exports = function createAffiliateApi(ctx) {
  ...
  function validCode(value) {
    return /^[A-Z0-9]{2,40}$/.test(normalizeCode(value));
  }
  ...
  async function getProfileByUserId(userId) {
    const rows = await supabaseRest(
      "GET",
      "profiles?user_id=eq." + encodeURIComponent(userId) + "&select=display_name,slug&limit=1"
    );
    return (rows && rows[0]) || null;
  }
  ...
  function isAutoGeneratedOAuthName(slug) {
    return /^USER[0-9A-F]{6,32}$/i.test(String(slug || ""));
  }

  function isUsableDisplayNameSlug(slug, rawName) {
    const raw = String(rawName || "").trim();
    const normalized = normalizeCode(slug);
    const rawSlug = slugFromDisplayName(raw);
    if (!raw) return false;
    if (!rawSlug || rawSlug.length < 2) return false;
    if (!/[A-Z]/.test(rawSlug)) return false;
    if (!validCode(normalized)) return false;
    if (isAutoGeneratedOAuthName(normalized)) return false;
    return normalized === rawSlug;
  }
  ...
  function generateRandomReferralCode() {
    ...
    return out + String(DISCOUNT_PERCENT);
  }

  async function preferredCodeFromDisplayName(user) {
    const rawName = displayNameFromUser(user, user && user.email);
    const slug = slugFromDisplayName(rawName);
    if (isUsableDisplayNameSlug(slug, rawName)) return normalizeCode(slug);

    try {
      const profile = await getProfileByUserId(user.id);
      const profileRaw = profile && profile.display_name;
      if (profileRaw && profileRaw !== rawName) {
        const profileSlug = slugFromDisplayName(profileRaw);
        if (isUsableDisplayNameSlug(profileSlug, profileRaw)) return normalizeCode(profileSlug);
      }
    } catch (err) {}
  ...
  async function allocateUniqueReferralCode(user, randomOnly) {
    const candidates = [];
    if (!randomOnly) {
      const fromName = await preferredCodeFromDisplayName(user);
      if (fromName) candidates.push(fromName);
    }
    for (let i = 0; i < 30; i += 1) {
      candidates.push(generateRandomReferralCode());
    }
    for (let j = 0; j < candidates.length; j += 1) {
      const code = normalizeCode(candidates[j]);
  ...
  async function ensureAffiliateForUser(user) {
    ...
    else affiliate = await ensurePromotionCode(affiliate);
    return affiliate;
  }
  ...
  async function createAffiliateForUser(user) {
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
      const code = await allocateUniqueReferralCode(user, attempt > 0);
      if (!validCode(code)) continue;
      try {
        ...
        const again = await getAffiliateByUser(user.id);
        if (again) return ensurePromotionCode(again);
      }
    }
  ...
  async function publicDashboard(user, origin) {
    var affiliate = await ensureAffiliateForUser(user);
  ...
  async function handleSyncAffiliate(req, res) {
    const user = await authUserFromReq(req);
    if (!user || !user.id) {
      sendJson(res, 401, { error: "Sign in to sync your referral code." });
      return;
    }
    try {
      const affiliate = await ensureAffiliateForUser(user);
      sendJson(res, 200, {
        ok: true,
        code: affiliate && affiliate.code ? affiliate.code : "",
        locked: true
      });
    } catch (err) {
      sendJson(res, err.status || 500, {
        error: err.message || "Could not sync referral code."
      });
    }
  }
  ...
  return {
    ...
    handleSyncAffiliate: handleSyncAffiliate,
    ensureAffiliateForUser: ensureAffiliateForUser,
